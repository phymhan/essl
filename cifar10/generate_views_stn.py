...
def stn_generate(theta, imgs, constraint=None):
    if constraint == 'clamp':
        ...
        theta1 = (torch.tanh(theta) + 1) / 2 * (SCALE_HIGH - SCALE_LOW) + SCALE_LOW
        mask = torch.zeros_like(theta)
        mask[:,0] = 1
        mask[:,4] = 1
        theta = theta + (theta1 - theta) * mask
    elif constraint == 'sigmoid':
        theta1 = torch.sigmoid(theta) * (SCALE_HIGH - SCALE_LOW) + SCALE_LOW
        mask = torch.zeros_like(theta)
        mask[:,0] = 1
        mask[:,4] = 1
        theta = theta + (theta1 - theta) * mask
...
if __name__ == '__main__':
    ...
    parser.add_argument('--log_root', type=str, default='logs_stn')
    ...
    parser.add_argument('--stn_init_val', type=float, default=1)
    parser.add_argument('--constraint', type=str, default=None)
    parser.add_argument('--init_from_mid', action='store_true')
    ...
    INIT_HIGH = not args.init_from_mid

    z = torch.zeros(batch_size, 6).to(device)  # NOTE: non-amortized
    if args.constraint == 'clamp':
        z[:,0] = 1 if INIT_HIGH else (SCALE_HIGH + SCALE_LOW) / 2
        z[:,4] = 1 if INIT_HIGH else (SCALE_HIGH + SCALE_LOW) / 2
    elif args.constraint == 'tanh':
        # z -= 5
        z[:,0] = 5 if INIT_HIGH else 0
        z[:,4] = 5 if INIT_HIGH else 0
    elif args.constraint == 'sigmoid':
        # z -= 5
        z[:,0] = 5 if INIT_HIGH else 0
        z[:,4] = 5 if INIT_HIGH else 0
    else:  # NOTE
        z[:,0] = args.stn_init_val
        z[:,4] = args.stn_init_val
    # z[:,2] = 1 - args.stn_init_val  # NOTE
    ...
    #                         truncation_latent=trunc, 
    #                         randomize_noise=False)
    imgs_rec = stn_generate(z[1::n, ...], imgs, constraint=args.constraint)
    # grid = F.affine_grid(z[1::n, ...].view(-1, 2, 3), imgs.size())
    # imgs_rec = F.grid_sample(imgs, grid)

    if args.clamp:
        imgs_rec = torch.clamp(imgs_rec, -1, 1)
    imgs_fakes = torch.cat([img_gen for img_gen in imgs_rec], dim=1)
    ...
    h_img = model(encoder_input_transform(imgs).repeat_interleave(n, dim=0))  # start from the reconstructed images
    ...
    for step in range(args.iters):
        ...
        #                         truncation_latent=trunc, 
        #                         randomize_noise=False)
        imgs_gen = stn_generate(z, imgs_rep, constraint=args.constraint)
        # grid = F.affine_grid(z.view(-1, 2, 3), img_rep_size)
        # imgs_gen = F.grid_sample(imgs_rep, grid)
        ...
        if args.debug:
            print('debug')
            z = z_orig + args.lr * torch.randn_like(z_orig)
